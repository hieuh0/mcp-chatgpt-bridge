...
- **`validateFetchModelsTarget`** — resolves the hostname via DNS and rejects loopback, link-local (including the `169.254.169.254` cloud-metadata address), RFC1918 private ranges, and CGNAT (`100.64.0.0/10`), for both a request-supplied `baseURL` and a previously-saved one used as a fallback. `validateBaseURL` alone only checks URL syntax/scheme, never the destination. The validated IP is then **pinned**: `fetchModelIds` connects directly to that address via `node:http`/`node:https` (not the global `fetch()`, which would re-resolve the hostname independently) while still setting the `Host` header and TLS `servername` to the original hostname, so certificate validation stays correct. Pinning closes a DNS-rebinding TOCTOU gap — without it, a validate-then-fetch sequence that re-resolves the hostname a second time could be shown a safe address for the check and a private/loopback one for the real connection, if the attacker controls that domain's DNS with a low TTL.
